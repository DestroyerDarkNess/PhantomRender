...
namespace $rootnamespace$
{
    internal sealed class UI : IDisposable
    {
        ...
        private void OnRender()
        {
            ...
            bool open = _visible;
            bool begin = HexaImGui.Begin("$safeprojectname$", ref open, ImGuiWindowFlags.AlwaysAutoResize);
            if (begin)
            {
        ...
        private static void DisposeInput(InputImguiEmu input)
        {
            if (input == null)
            {
                return;
            ...
            try
            {
                input.GetType().GetMethod("Dispose", Type.EmptyTypes)?.Invoke(input, null);
            }
            catch
            {
            }
